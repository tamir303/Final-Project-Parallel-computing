...
Point *readPointArrayFromFile(char *fileName, Info **info)
{
    ...
    *info = (Info *) malloc(sizeof(Info));
    ...
    if (!info)
    {
        printf("Faild to initiate info general.c line {%d}\n", __LINE__);
        exit(-1);
    }

    if (!file)
    {
        printf("Faild to open file general.c line {%d}\n", __LINE__);
        exit(-1);
    }

    if (!fscanf(file, "%d %d %lf %d", &((*info)->N), &((*info)->K), &((*info)->D), &((*info)->tCount)))
    {
        printf("Faild to read info general.c line {%d}\n", __LINE__);
        exit(-1);
    }
    ...
    while (count < (*info)->N)
    {
        ...
        {
            printf("Failed to read point {%d} general.c line {%d}\n", count, __LINE__);
            exit(-1);
        }
        count++;
    }
